...
timepoint=array[6]
output="/scratch1/migomez/3Dmodels/models/Fragram/{}/".format(timepoint)


## Point to the scalebar and coordinate files
scalebars_path = "/project/ckenkel_26/software/metashape-pro/Scales/FragramScale_050625.txt"
FragramCoords = "/project/ckenkel_26/software/metashape-pro/Scales/FragramCoords_050625.txt"
...
photoset=glob.glob(cwd+"/*.bmp") 	# directs metashape to photos to be used to build the model. There is a chance you have to change to .jpg/.JPG
...
# Set
chunk.importReference(path = FragramCoords, format = Metashape.ReferenceFormatCSV, delimiter = ',', columns = "nXYZxyz", skip_rows = 2,
...
#center = (0,0,0.068) # (X,Y,Z) in CRS coordinates - could do (0,0,0.1) to move the box up
#size = (0.1,0.1,0.11) # (width,depth,height)in meters
#angle = 90 # float - rotation angle of the region (in degrees)

# If using MY block and you want to include the entire thing
#center = (0,0,0.02) # (X,Y,Z) in CRS coordinates - could do (0,0,0.1) to move the box up
