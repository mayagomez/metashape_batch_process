...
TIMEPOINT=$1
TEMPLATE=reefscale_template.py	# Option to change this to the template script of your choosing.
DIR=/scratch1/migomez/3Dmodels/source_images/$TIMEPOINT/Reefscale	# Option to change this location. All you should need to do is change migomez to your username.
...
#mkdir /project/ckenkel_26/software/metashape-pro/ToRun/$TIMEPOINT/Reefscale          # Option to change this location.
OUT=/project/ckenkel_26/software/metashape-pro/ToRun/$TIMEPOINT/Reefscale     	# Option to change this location.
...
if [ "$#" -ne 1 ]
        ...
        echo "For example: ./buildscripts.sh ORCC_October202'"
        echo ""
        exit
fi
...
for file in `cat $OUT/listofscripts.txt`; do
   ...
   #sed -i 's#HELLO#"/project/ckenkel_26/software/metashape-pro/Scales/Adjustable$ADJUSTABLEScale_011623.txt"#' $file
done
...
for i in `ls ToRun/$TIMEPOINT/Reefscale/*.py`; do		# This could also be done as follows: for i in `cat $OUT/listofscripts.txt`; do 
...
cat template_MetashapeJobSubmit.slm $OUT/ToDo.txt > ${TIMEPOINT}_Reefscale_MetashapeJobSubmit.slm
chmod +x ${TIMEPOINT}_Reefscale_MetashapeJobSubmit.slm
