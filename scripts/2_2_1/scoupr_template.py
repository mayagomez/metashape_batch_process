...
project = "AcroFLaT"		 # replace this if changing projects!

## Set output filepath based on the 5th element in the source_images filepath
array=source_images.split('/')
timepoint=array[6]
scoupr=array[7]
output=f"/scratch1/migomez/3Dmodels/models/{project}/{timepoint}/{scoupr}/"

## Point to the scalebar and coordinate files
scalebars_path = "/scratch1/migomez/metashape-pro_2_2_1/Scales/BARSTICKERS_060123.txt"
#scalebars_path = "/scratch1/migomez/metashape-pro_2_2_1/Scales/FragramScale_CRL.txt"
...
#Adjustable1BScale_041823.txt  Adjustable3Scale_BARSTICKERS_060123.txt  RackCoords_012423.txt  RackScale_011723.txt	  BARSTICKERS_060123.txt
...
photoset=glob.glob(cwd+"/*.JPG") 	# directs metashape to photos to be used to build the model. There is a chance you have to change between .jpg/.JPG/.bmp/.png
...
# generic_preselection = True saves time. However, if not limited by processing power (ie. HPC) then generic_preselection = False is preferred. It takes longer but results in fewer situations wher$
...
# NOTE: Tolerance is how sensitive the algorithm is to detecting targets. Range is 0 - 100. If tolerance is high, then there is risk of overdetecting markers (ie. thinks something is a $
...
### 6. Optimize camera alignment ---- NOT NECESSARY FOR FRAGRAM MODELS
