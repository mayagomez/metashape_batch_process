...
# If using my block:
scalebars_path = "/scratch1/migomez/metashape-pro_2_2_1/Scales/FragramScale_050625.txt"
FragramCoords = "/scratch1/migomez/metashape-pro_2_2_1/Scales/FragramCoords_050625.txt"

# If using CRL's Block #2:
#scalebars_path = "/scratch1/migomez/metashape-pro_2_2_1/Scales/FragramScale_CRLBlock2.txt"
#FragramCoords = "/scratch1/migomez/metashape-pro_2_2_1/Scales/FragramCoords_CRLBlock2.txt"
...
photoset=glob.glob(cwd+"/*.bmp") 	# directs metashape to photos to be used to build the model. There is a chance you have to change between .jpg/.JPG/.bmp/.png
...
chunk.matchPhotos(downscale = 1, keypoint_limit_per_mpx = 800, generic_preselection = False, reference_preselection=False, filter_mask=False, mask_tiepoints=True,
			filter_stationary_points=False, keypoint_limit=150000, tiepoint_limit=40000, keep_keypoints=False, guided_matching=True,
                    reset_matches=True, subdivide_task=True, workitem_size_cameras=20, workitem_size_pairs=80, max_workgroup_size=100)
doc.save()
...
# If using MY block and you want to include the entire block 
center = (0,0,0.02) # (X,Y,Z) in CRS coordinates
#center = (0,0,0.087) # swap to this for autotrimming
#size = (0.13,0.13,0.15) # (width,depth,height) in meters --- CHANGE THIS TO MODIFY THE SIZE OF YOUR BUILD. Used for this for all single polyp work..
size = (0.09,0.09,0.12) # used to have 0.1, 0.1, 0.14 -- too small for some frags --- used.
#size = (0.1,0.1,0.11) # (width,depth,height) in meters --- trying for ofavs
#size = (0.1,0.1,0.1) # (width,depth,height) in meters --- trying for ofavs
angle = 90 # float - rotation angle of the region (in degrees)
