...
# PURPOSE: This script uses provided photosets to build scaled models of up to 10 coral colonies contained within a rack system via command line Agisoft Metashape. Scales are based on a coordinate system created with the rack frame.
	 # The output models are trimmed of the surrounding sand/space leaving only the top of the rack and corals for analysis.
...
center = (0,0,0) # (X,Y,Z) in CRS coordinates - could do (0,0,0.1) to move the box up
size = (0.7,0.7,0.4) # (width,depth,height)in meters
