...
TIMEPOINT=$1
DIR=/scratch1/migomez/3Dmodels/source_images/$TIMEPOINT/Rack	# Option to change this location. All you should need to do is change migomez to your username.
TEMPLATE=rackfull_template_scales.py  				# Option to change this to the template script of your choosing. Ex. racktrim_template.py or rackfull_template.py
...
#mkdir /project/ckenkel_26/software/metashape-pro/ToRun/$TIMEPOINT/Rack          # Option to change this location.
OUT=/project/ckenkel_26/software/metashape-pro/ToRun/$TIMEPOINT/Rack     	# Option to change this location.
...
for line in `cat $OUT/PhotosetDirs.txt`; do
    ...
    name1="${NAME[7]}";				# this picks the 7th thing in the file path and names the output .py as such
...
for i in `ls ToRun/$TIMEPOINT/Rack/*.py`; do		# This could also be done as follows: for i in `cat $OUT/listofscripts.txt`; do 
...
cat template_MetashapeJobSubmit.slm $OUT/ToDo.txt > ${TIMEPOINT}_Rack_MetashapeJobSubmit.slm
chmod +x ${TIMEPOINT}_Rack_MetashapeJobSubmit.slm
